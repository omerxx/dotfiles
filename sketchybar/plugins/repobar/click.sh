...
open -g -b com.steipete.repobar >/dev/null 2>&1 || open -ga "RepoBar" >/dev/null 2>&1 || true

for _ in {1..20}; do
  if pgrep -x "RepoBar" >/dev/null 2>&1; then
    break
  fi
  sleep 0.1
done
...
tell application "System Events"
  if not (exists process "RepoBar") then return

  tell process "RepoBar"
