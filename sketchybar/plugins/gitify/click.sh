...
# Ensure Gitify is running
if ! pgrep -x "Gitify" >/dev/null 2>&1; then
  open -g -a "Gitify" >/dev/null 2>&1 || true
  for _ in {1..20}; do
    pgrep -x "Gitify" >/dev/null 2>&1 && break
    sleep 0.1
  done
fi
...
tell application "System Events"
  if not (exists process "Gitify") then return
