...
update_workspace_windows() {
    ...
    if [ "${apps}" != "" ]; then
        ...
        sketchybar --set space.$workspace_id label="$icon_strip" label.drawing=on drawing=on
        ...
        if [ "$FOCUSED_WORKSPACE" = "$workspace_id" ]; then
            sketchybar --set space.$workspace_id label.drawing=off background.drawing=on background.color=0xffcba6f7 drawing=on
        else
            sketchybar --set space.$workspace_id label.drawing=off background.drawing=off drawing=off
        fi
    fi
}
