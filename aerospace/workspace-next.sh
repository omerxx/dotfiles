...
# Focus the next window; if at the edge, switch to the next workspace and focus the first window there.
#
# Tunables (optional):
...
if [[ "$cooldown_ms" =~ ^[0-9]+$ ]] && ((cooldown_ms > 0)); then
  lock_dir="${TMPDIR:-/tmp}/aerospace-thumbwheel-next.lock"
  if ! mkdir "$lock_dir" 2>/dev/null; then
    exit 0
  fi
  trap 'rmdir "$lock_dir" 2>/dev/null || true' EXIT

  ts_file="${TMPDIR:-/tmp}/aerospace-thumbwheel-next.ts"
...
if [[ -z "$focused_window_id" ]]; then
  ...
  "$AEROSPACE" focus --dfs-index 0 >/dev/null 2>&1 || exit 0
fi

if "$AEROSPACE" focus dfs-next --boundaries-action fail >/dev/null 2>&1; then
  exit 0
fi
...
for i in "${!candidate_workspaces[@]}"; do
  if [[ "${candidate_workspaces[$i]}" == "$current_workspace" ]]; then
    next_index=$(((i + 1) % ${#candidate_workspaces[@]}))
    target_workspace="${candidate_workspaces[$next_index]}"
    break
  fi
done

if [[ -z "$target_workspace" ]]; then
  target_workspace="${candidate_workspaces[0]}"
fi
